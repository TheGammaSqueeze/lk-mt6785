 ...
 * scrolling diagonal rainbow. The renderer lives in dev/video/mtk_cfb.c because
 * it must draw into the LK console framebuffer (pGD->frameAdrs) - the buffer the
 * running video-mode display path re-scans on every mt_disp_update(), the same

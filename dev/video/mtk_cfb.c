...
/*
 * AYANEO experiment: scrolling diagonal rainbow painted into the LK console
 * framebuffer (pGD->frameAdrs), which is the buffer the running video-mode
 * display path actually re-scans on every mt_disp_update(). The static boot
 * logo is drawn into the FB_LAYER buffer, which is only latched once, so
 * animating that buffer does not work; the console/boot-menu buffer is the same
 * one the fastboot menu and the AVB orange-state warning redraw through, so
 * per-frame updates are picked up. 32bpp only (PIXEL_SIZE == 4).
 */
...
void video_rainbow_boot(void)
{
	...
	unsigned int i, f, x, y, W, H;

	/* make sure the console framebuffer (pGD->frameAdrs) is set up */
	drv_video_init();
	if (cfb_fb_addrs == NULL || PIXEL_SIZE != 4)
		return;
	...
	for (f = 0; f < AYANEO_RAINBOW_FRAMES; f++) {
		...
		for (y = 0; y < H; y++) {
			unsigned int *row = (unsigned int *)
				((unsigned char *)cfb_fb_addrs + y * LINE_LEN);

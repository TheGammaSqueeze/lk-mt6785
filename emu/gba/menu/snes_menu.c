...
/* game at carousel slot i, honouring the current sort order. In GBA mode there is
 * no pack game record (the roster is the SD ROM list), so this returns 0 and all
 * per-game record fields (boxart/player-icon/sort) are bypassed by the callers. */
static const snes_game_rec *game(const snes_menu *m, int i)
{
	int n = m->ngames;
	if (n <= 0 || m->gba_mode) return 0;
	i = ((i % n) + n) % n;
	return game_raw(m->pk, m->order[i]);
}
/* display name for carousel slot i (GBA ROM name, or the pack game name) */
static const char *game_name(const snes_menu *m, int i)
{
	int n = m->ngames;
	if (n <= 0) return "";
	i = ((i % n) + n) % n;
	if (m->gba_mode) return m->gba_names ? m->gba_names[m->order[i]] : "";
	return snes_str(m->pk, game_raw(m->pk, m->order[i])->name);
}
			      ...
			      const snes_img_entry *cart)
{
	...
	m->ngames = n;
	m->sort_rule = 0;
	for (i = 0; i < n; i++) m->order[i] = (unsigned short)i;
...
void snes_menu_update(snes_menu *m, const snes_input *in, float dt)
{
	...
	if (m->state == 0) {                       /* ---- home carousel ---- */
		...
		/* A or Start launches the focused ROM (GBA); SNES build keeps it stubbed */
		if (ea || (in->start && !m->pstart)) {
			push_snd(m, m->sfx_decide);
			if (m->gba_mode) {
				int n = m->ngames, f = ((m->focus % n) + n) % n;
				m->launch = m->order[f];
			}
		}
	...
	m->pa = in->a; m->pb = in->b; m->ps = in->select; m->pstart = in->start;

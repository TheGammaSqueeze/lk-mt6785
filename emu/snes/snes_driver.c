			...
			snes_menu *menu, snes_target *tfull)
{
	...
	*(volatile unsigned *)(unsigned long)0x10006250u = 0x5A5A0000u | (seq & 0xffffu);
	{	/* FULL 23-word state channel: pack the real menu dynamic state and publish it to the
		...
		g_bc_state_sum = ssum;
		/* DISAMBIGUATE frozen-read vs dropped-write: cpu0 reads back its own MMIO writes. If
		 * these show 0x5A5A/the packed word, cpu0's writes LAND and the worker's 0 is a frozen
		 * READ (MMIO snapshot). If these read 0 too, cpu0's SPM scratch write DROPPED (pick a
	...
	for (;;) {
...

		{
			...
			if (layered) {
				...
				/* leaving the layered state: disable the upper OVL layers ONCE, forced to a
				 * vblank boundary (see the transition skip_vsync above), so no stale linger. */
